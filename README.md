...
Node 20.19 이상(또는 22.12 이상)과 pnpm 11 이상이 필요하다. `.nvmrc`에 기준 버전이 있다.
pnpm 10 이하에서는 `pnpm-workspace.yaml`의 `allowBuilds`가 인식되지 않아 lint가 깨진다.
...
- **목록 스크롤 위치는 히스토리 항목 단위로 복원.** 뒤로가기로 돌아오면 보던
  ...
  갈 수 있는 데까지만 간다. 한 건을 지우면 줄어드는 높이가 카드 한 장이라,
  맨 끝에서 마지막 항목을 지워도 새 맨 끝에 남는다. 복원을 포기하고 최상단으로
  보내는 쪽도 검토했지만, 500건 중 하나를 지웠다고 보던 자리를 잃는 편이 더
  나쁘다고 봤다. 잘라내는 일은 브라우저와 윈도우 계산이 이미 하므로 따로
  더한 코드는 없다. 위치는 메모리에만 있어 새로고침하면 초기화된다.
- **상세에서 목록으로는 뒤로가기로 이동.** 보던 위치를 지키기 위해서다. 딥링크로
  들어와 돌아갈 항목이 없을 때만 목록 경로로 보낸다. 로그인을 거쳐 상세에 들어온
  경우를 위해 로그인 성공은 push가 아니라 replace로 처리한다. 그러지 않으면
  뒤로가기 한 칸이 목록이 아니라 로그인 화면이 된다.
- **삭제 후에는 뒤로가기를 쓰지 않고 목록 경로로 보낸다.** 요구사항이 "목록으로
  redirect"를 문구로 정했는데, 뒤로가기는 직전 항목이 목록이라는 가정에 기댄다.
  대시보드나 회원정보를 거쳐 상세에 들어왔다면 그 가정이 깨져 엉뚱한 화면에
  떨어지고, 삭제 안내도 목록에서만 뜨므로 함께 유실된다. replace로 보내 지운
  항목의 상세가 히스토리에서 빠지는 대신, 삭제 후 목록은 최상단에서 시작한다.
